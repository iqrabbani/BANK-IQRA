...
public interface NasabahRepository extends JpaRepository<Nasabah,String>{
    @Query("" +
            "SELECT nas FROM Nasabah nas WHERE nas.nomorKtp = %:nomorKtp%" +
            "")
    List<Nasabah> getAllByKtp(String nomorKtp);

    @Query("""
            SELECT COUNT(nas) FROM Nasabah nas
            WHERE
                nas.nomorRekening = %:nomorRekening%
            """)
    int countNoRekening(String nomorRekening);
}
